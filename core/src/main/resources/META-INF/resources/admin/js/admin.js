        ...
        // Setup workspaces polling (SSE removed to avoid HTTP/1.1 connection limits)

        // Load workspaces (initial load only, SSE will keep it updated)
        async function loadWorkspaces() {
            try {
                const response = await fetch('/api/admin/workspaces');
                const newWorkspaces = await response.json();

                // Check if data actually changed to avoid unnecessary re-renders
                if (JSON.stringify(newWorkspaces) !== JSON.stringify(workspaces)) {
                    ...
                    // Refresh servers for currently selected workspace if it still exists
                    if (selectedWorkspace) {
                        const workspace = workspaces.find(w => w.rootUri === selectedWorkspace);
                        if (workspace) {
                            renderServers(workspace.lspServers);
                ...
                return `
                    <div class="workspace-item ${client.id === selectedMcpClient ? 'active' : ''}"
                        ...
                        <div style="font-weight: 600; margin-bottom: 0.25rem;">
                            📱 ${escapeHtml(client.name)} ${timeStr ? `<span style="color: #666; font-weight: normal; font-size: 0.75rem;">@ ${timeStr}</span>` : ''}
                        </div>
        ...
        async function loadMcpConsole(clientId) {
            ...
            // SSE is already set up in loadMcpTracesConsole() - no need to duplicate it here
        }
        ...
        function renderMcpConsoleWithHighlights() {
            ...
            const html = clientTraces.map(trace => formatMcpTrace(trace, currentSearchQuery)).join('');
            output.innerHTML = html;
            ...
            return `
                <div class="trace-line" onmouseenter="showMcpTooltip(event, '${traceId}', ${!hasMatch})" onmouseleave="hideMcpTooltip('${traceId}')">
                    <div style="display: flex; align-items: flex-start; padding: 0.25rem; cursor: pointer;"
                        ...
                        <div style="flex: 1; font-family: 'Consolas', 'Monaco', monospace; font-size: 0.85rem; color: #cccccc;">${highlightText(headerLine, searchQuery)}</div>
                    </div>
                    <div id="${traceId}" class="mcp-trace-body ${foldState}" style="font-family: 'Consolas', 'Monaco', monospace; font-size: 0.8rem; color: #d4d4d4; white-space: pre-wrap; word-wrap: break-word; padding-left: 1.5rem;">${highlightText(body, searchQuery)}</div>
                    <div class="trace-tooltip" id="${traceId}-tooltip">${escapeHtml(fullContent)}</div>
                </div>
            `;
        }
        ...
        function toggleAllMcpTraces() {
            toggleAllTracesGeneric('mcp-console-output', 'mcp-trace-body', 'mcp-trace-toggle', 'mcp-fold-button', {
